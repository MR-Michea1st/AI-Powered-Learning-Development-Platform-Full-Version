...
from courses.models import Track,Course,Enrollment,TrackEnrollment , InterviewResults
from courses.serializers import EnrollmentSerializer,EnrollmenTracktSerializer , InterviewResultsSerializer
...
class ListUserInterviewsView(ListAPIView):
    """
    List all interviews completed by the authenticated user.
    Supports filtering by track_name and sorting by interviewed_at.
    """
    serializer_class = InterviewResultsSerializer
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.OrderingFilter]
    filterset_fields = ['track_name']
    ordering_fields = ['interviewed_at', 'percentage_result']
    ordering = ['-interviewed_at']

    def get_queryset(self):
        return InterviewResults.objects.filter(user_id=self.request.user).order_by('-interviewed_at')